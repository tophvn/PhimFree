...
<body>
    ...
    <div class="container">
        ...
        error_log("API URL (phim-le.php): " . $api_url);
        ...
        if ($data && isset($data['data']['items']) && !empty($data['data']['items'])) {
                ...
                echo "
                    <div class='movie-card'>
                        <a href='phim.php?slug={$slug}'>
                            <img src='{$thumbnail}' alt='{$title}' onerror=\"this.src='{$fallback_thumbnail}'; console.log('Lỗi tải ảnh (phim-le.php): {$thumbnail}');\">
                            <div class='info'>
                                <h3>{$title}</h3>
                                <p>Năm: {$year}</p>
                            </div>
            ...
            error_log("Phân trang (phim-le.php): totalItems=$total_items, totalPages=$total_pages, currentPage=$page, itemsCount=" . count($movies));

            if ($total_pages > 1) {
                echo '<div class="pagination">';
                if ($page > 1) {
                    echo "<a href='phim-le.php?page=" . ($page - 1) . "'>Trang trước</a>";
                } else {
                    echo "<span class='disabled'>Trang trước</span>";
                }
                echo " <span>Trang $page / $total_pages</span> ";
                if ($page < $total_pages) {
                    echo "<a href='phim-le.php?page=" . ($page + 1) . "'>Trang sau</a>";
                } else {
            ...
            error_log("Lỗi API tại phim-le.php: " . json_encode($data));
        }

        $end_time = microtime(true);
        error_log("Thời gian xử lý phim-le.php: " . ($end_time - $start_time) . " giây");
        ?>
    </div>
</body>
</html>